...
func TestDuration(t *testing.T) {
	...
	v := Duration(d)
	if v.String() != s {
		t.Errorf("expected %s, but received %s", s, v.String())
	}
}

func TestDurationSlice(t *testing.T) {
	s1 := "30s"
	d1, _ := time.ParseDuration(s1)
	s2 := "1m0s"
	d2, _ := time.ParseDuration(s2)
	s := []time.Duration{d1, d2}
	ps := DurationSlice(s)
	if len(ps) != 2 {
		t.Errorf("expected %d, but received %d", 2, len(ps))
	}
	if ps[0].String() != s1 {
		t.Errorf("expected %s, but received %s", s1, ps[0].String())
	}
	if ps[1].String() != s2 {
		t.Errorf("expected %s, but received %s", s2, ps[1].String())
	}
}
...
func TestDurationMap(t *testing.T) {
	...
	if ps[s1].String() != s1 {
		t.Errorf("expected %s, but received %s", s1, ps[s1].String())
	}
	if ps[s2].String() != s2 {
		t.Errorf("expected %s, but received %s", s2, ps[s2].String())
	}
}
